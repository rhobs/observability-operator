...
type Overlay struct {
	...
	overlayDir    string
	clusterScoped map[resid.Gvk]bool
}
...
// SetNamespace sets the default namespace applied to namespaced resources that
// lack one. Cluster-scoped resources registered with [SetClusterScoped] are
// left untouched.
func (o *Overlay) SetNamespace(ns string) {
	o.namespace = ns
}

// SetClusterScoped marks a GVK as cluster-scoped so the namespace transform
// skips it. Call this for custom cluster-scoped CRDs that kustomize doesn't
// recognise (core types like ClusterRole are handled automatically).
func (o *Overlay) SetClusterScoped(group, kind string) {
	if o.clusterScoped == nil {
		o.clusterScoped = make(map[resid.Gvk]bool)
	}
	o.clusterScoped[resid.Gvk{Group: group, Kind: kind}] = true
}
...
func (o *Overlay) buildResMap() (resmap.ResMap, error) {
	...
	if o.namespace != "" {
		...
		for _, r := range resMap.Resources() {
			if o.clusterScoped[resid.Gvk{Group: r.GetGvk().Group, Kind: r.GetGvk().Kind}] {
				continue
			}
...
func (o *Overlay) Build() ([]client.Object, error) {
	...
	for _, res := range resMap.Resources() {
		...
		if o.ownerName != "" {
			util.AddCommonLabels(obj, o.ownerName)
		}
...
func (o *Overlay) writeOverlayWith(write fileWriter) error {
	...
	sortedResources := slices.Sorted(maps.Keys(o.resources))
	sortedPatches := slices.Sorted(maps.Keys(o.patches))

	kust.Resources = append(kust.Resources, sortedResources...)
	for _, path := range sortedPatches {
	...
	for _, path := range sortedResources {
