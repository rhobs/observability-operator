...
func (cfg *OverlayConfig) Capabilities(cc obsv1alpha1.CommonCapabilitiesSpec) {
	cfg.Resources = cc.Enabled
	cfg.Operators = cc.InstallOperators()
}
...
func BuildOverlay(instance *obsv1alpha1.ObservabilityInstaller, cfg OverlayConfig) (*overlay.Overlay, error) {
	...
	if tracing := instance.Spec.Capabilities.Tracing; tracing != nil {
		cfg.Capabilities(tracing.CommonCapabilitiesSpec)
		if cfg.Operators || cfg.Resources {
			if cfg.Operators {
...
// ResolveInstallerObjects builds the object set that reconciles an
...
// objects.
func ResolveInstallerObjects(ctx context.Context, reader client.Reader, instance *obsv1alpha1.ObservabilityInstaller, cfg OverlayConfig) ([]client.Object, error) {
	o, err := BuildOverlay(instance, cfg)
	if err != nil {
