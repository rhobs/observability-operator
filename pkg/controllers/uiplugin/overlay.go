...
// ApplyTLSProfile fills the TLSMinVersion and TLSCiphers fields of the build
// config from an OpenShift TLS security profile. OpenSSL cipher names from the
// profile are converted to the IANA names expected by the plugin server's
...
func BuildOverlay(plugin *uiv1alpha1.UIPlugin, conf UIPluginBuildConfig, logger logr.Logger) (*overlay.Overlay, *UIPluginInfo, error) {
	...
	case uiv1alpha1.TypeDistributedTracing:
		...
		if pluginInfoErr == nil && conf.TempoServiceNames != nil {
			pluginInfo.TempoServiceNames = conf.TempoServiceNames
			for ns, svc := range conf.TempoServiceNames {
				if svc != "" {
					pluginInfo.Proxies[0].ServiceName = svc
					pluginInfo.Proxies[0].ServiceNamespace = ns
					break
				}
			}
		}
...
// ResolvePluginObjects builds the operand object set for a UIPlugin: the
...
func ResolvePluginObjects(plugin *uiv1alpha1.UIPlugin, conf UIPluginBuildConfig, logger logr.Logger) ([]client.Object, *UIPluginInfo, error) {
