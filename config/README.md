...
Some special placeholder values are replaced automatically during reconcile and generation.

ObservabilityInstaller placeholders:
- `placeholder-observability-installer-name`: Name derived from the ObservabilityInstaller name.
- `placeholder-observability-installer-namespace`: The ObservabilityInstaller namespace.
- `placeholder-operator-namespace`: The namespace where the operator is installed.

UIPlugin placeholders:
- `placeholder-uiplugin-name`: Resolved name for the UIPlugin operand resources.
- `placeholder-uiplugin-crname`: Name of the UIPlugin custom resource.
...
- `pkg/controllers/observability/tracing_overlay.go` — adds tracing components
  (OTEL collector, TempoStack, UIPlugin, Subscriptions) with dynamic patches.
- `pkg/controllers/uiplugin/overlay.go` — `BuildUIPluginOverlay()` entry point that
  builds UIPlugin operand resources (Deployment, Service, etc.) from kustomize components.
...
Placeholder values are replaced with real values via string substitution.
`Build` and `BuildYAML` apply substitutions after the kustomize build.
`WriteToDir` applies substitutions while writing the bundle so that
the exported files can be built directly with `kustomize build`.
