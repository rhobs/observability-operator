...
parse_args() {
  ...
  while [[ -n "${1+xxx}" ]]; do
    ...
    --no-builds)    NO_BUILDS=true; shift ;;
    --ci)           CI_MODE=true; shift ;;
    --image-repo)   shift
      OBO_IMG_REPO="$1";
      BUNDLE_IMG="$OBO_IMG_REPO-bundle:$OBO_VERSION"
      shift ;;
    --ns)           shift; OPERATORS_NS=$1; shift ;;
...
wait_for_operators_ready(){
  kubectl wait -n "$OPERATORS_NS" --for=condition=Available deploy/observability-operator --timeout=300s
  kubectl wait -n "$OPERATORS_NS" --for=condition=Available deploy/obo-prometheus-operator --timeout=300s
