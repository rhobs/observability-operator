...
func clusterHealthAnalyzer(t *testing.T) {
	...
	err = f.K8sClient.Create(t.Context(), dep)
...
func newMonitoringUIPlugin(t *testing.T) *uiv1.UIPlugin {
	...
	if err == nil {
		...
		f.K8sClient.Delete(t.Context(), existing)
	...
	f.CleanUp(t, func() {
		f.K8sClient.Delete(t.Context(), plugin)
...
func newCrashLoopRule(t *testing.T, ruleName, alertName, podPrefix string) *monv1.PrometheusRule {
	...
	f.CleanUp(t, func() {
		f.K8sClient.Delete(t.Context(), rule)
	})
	return rule
}
...
func newCrashingDeployment(t *testing.T, name string) *appsv1.Deployment {
	...
	f.CleanUp(t, func() {
		f.K8sClient.Delete(t.Context(), dep)
	})
	return dep
}
